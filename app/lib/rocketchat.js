...
const RocketChat = {
		...
		return new Promise((resolve) => {
			...
			this.ddp.on('stream-notify-logged', (ddpMessage) => {
				...
				if (ddpMessage.fields && ddpMessage.fields.eventName === 'updateAvatar') {
					...
					database.write(() => {
						args.forEach((arg) => {
							const user = database.objects('users').filtered('username CONTAINS[c] $0', arg.username);
							if (!user.length) {
	...
	*/
	...
	setAvatarFromService({ url, contentType = '', service = null }) {
		return call('setAvatarFromService', url, contentType, service);
	}
};
